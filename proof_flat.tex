...
\textbf{Exact chromatic number and theta-perfectness.}  We prove that the GQ witness graph $G_q$ is \emph{theta-perfect}: $\omega(G_q)=\vartheta(\overline{G_q})=\chi_f(G_q)=\chi(G_q)=q^3+1$, where $\vartheta$ denotes the Lov\'asz theta function.  In particular, $\chi(G_q)=q^3+1$ \emph{exactly} (not just $\ge$), achieved via a spread coloring of $\mathrm{GQ}(q,q^2)$.  The small value $\vartheta(G_q)=q+1=O(d_q^{1/3})$ shows the SDP-independence bound is tight; the linear conjecture $M(n)=O(n)$ is equivalent to $\alpha(G)\ge |V(G)|/(Cn)$ for all sphere packing contact graphs $G$ in $\mathbb{R}^n$.
\end{abstract}
...
The coloring $\varphi(a) = \sum_i i \cdot a_i \bmod (n+1)$ partitions $A_n$ into $n+1$ cosets of the sublattice $V_0 = \ker\varphi \le A_n$ with $[A_n : V_0] = n+1$.  Two vertices $u, v$ in the same coset satisfy $\varphi(v-u) = 0$, so $v - u \notin S$: the cosets are independent sets.
\end{remark}
...
Apply Proposition~\ref{prop:coloring-criterion} with $k = 2n-1$ and $c_i = i-1$ (so $c_1=0, c_2=1, \ldots, c_n=n-1$):
\[
...
\]
The four sign cases give values $(i-1)+(j-1) = i+j-2 \in \{1,\ldots,2n-3\}$, $-(i+j-2) \equiv 2n+1-i-j \in \{2,\ldots,2n-2\}$, $(i-1)-(j-1)=i-j \equiv i-j+(2n-1) \in \{n,\ldots,2n-2\}$, and $j-i \in \{1,\ldots,n-1\}$, all nonzero modulo $2n-1$.  Hence $\varphi(s) \not\equiv 0$ for all $s \in S$, giving $\chi(G_{D_n}) \le 2n-1$.
\end{proof}

\begin{remark}[Tightness]
The bound $2n-1$ is not always tight: $\chi(G_{D_2}) = 2 < 3$ (bipartite), and $\chi(G_{D_3}) = 4 < 5$ since $D_3 \cong A_3$ (face-centred cubic).  For $n \ge 4$ the exact value of $\chi(G_{D_n})$ is open; achieving $k < 2n-1$ via Proposition~\ref{prop:coloring-criterion} requires $n$ pairwise non-antipodal elements of $\mathbb{Z}_k$, which demands $k \ge 2n-1$ by a counting argument.
\end{remark}
...
\begin{enumerate}
  \item[\textup{(a)}] \textbf{$E_8$ ($n=8$):}  The restriction of the $A_8$ coloring $\varphi(v) = \sum_i i\,v_i \bmod 9$ (expressed in the standard coordinate representation) avoids zero on all $240$ minimum vectors of $E_8$~\cite{BvM2022}.  A $9$-clique exists from an $A_8$ root simplex, so $\chi(G_{E_8}) = 9 = n+1$.
  \item[\textup{(b)}] \textbf{Leech lattice $\Lambda_{24}$ ($n=24$):}  A $(25 = n+1)$-coloring arises from an index-$25$ sublattice of $\Lambda_{24}$.  The equidistant-set bound $\omega \le n+1 = 25$ is achieved, giving $\chi(G_{\Lambda_{24}}) = 25 = n+1$.
\end{enumerate}
\end{remark}
...
$n$ & Lattice & $\chi(G_\Lambda)$ & $n+1$ \\ \hline
$1$ & $A_1 = \mathbb{Z}$ & $2$ & $2$ \\
$2$ & $A_2$ (hexagonal) & $3$ & $3$ \\
$3$ & $A_3 \cong D_3$ (f.c.c.) & $4$ & $4$ \\
$n$ & $A_n$ & $n+1$ & $n+1$ \\
$n$ & $D_n$ & $\le 2n-1$ & $n+1$ \\
$8$ & $E_8$ & $9$ & $9$ \\
$24$ & $\Lambda_{24}$ (Leech) & $25$ & $25$ \\
\end{tabular}
\caption{Lattice contact graph chromatic numbers.  In all known cases, $\chi(G_\Lambda) = n+1$.}
\end{table}

\begin{conjecture}[Lattice packing chromatic bound]\label{conj:lattice-linear}
For every lattice sphere packing $\Lambda$ in $\mathbb{R}^n$, $\chi(G_\Lambda) \le n + 1$.
\end{conjecture}

If Conjecture~\ref{conj:lattice-linear} holds, the supremum $M(n)$ is determined by non-lattice packings.  Combined with the GQ lower bound $M(d_q) \ge q^3+1 = d_q + \Theta(d_q^{2/3})$ and an upper bound $M(n) \le n + O(n^{2/3})$ (currently unproved), this would give the strong asymptotic $M(n) = n + \Theta(n^{2/3})$.

\begin{remark}[Connection to independence density]\label{rem:indep-density}
For a vertex-transitive Cayley graph on an infinite abelian group, $\chi_f(G_\Lambda) = 1/\alpha^*$ where $\alpha^*$ is the upper density of a maximum periodic independent set.  Conjecture~\ref{conj:lattice-linear} is equivalent to: \emph{every lattice $\Lambda$ in $\mathbb{R}^n$ admits a periodic independent set of relative density $\ge 1/(n+1)$}.  For $A_n$, the sublattice $V_0 = \ker\varphi$ achieves this density exactly.
\end{remark}
...
\item[\textup{(iv)}] \textbf{Exact chromatic number and theta-perfectness:} For every prime power $q\ge 2$,
\[
  \omega(G_q) \;=\; \vartheta(\overline{G_q}) \;=\; \chi_f(G_q) \;=\; \chi(G_q) \;=\; q^3+1,
\]
where $\vartheta$ is the Lov\'asz theta function.  In particular, the lower bound $M(d_q)\ge q^3+1$ is realised by a packing whose tangency graph $G_q$ has chromatic number exactly $q^3+1$ (achieved via a spread coloring of $\mathrm{GQ}(q,q^2)$).  Equivalently, Conjecture~\ref{conj:linear} ($M(n)=O(n)$) holds if and only if $\alpha(G)\ge |V(G)|/(Cn)$ for every sphere packing contact graph $G$ in $\mathbb{R}^n$.
\end{enumerate}
\end{corollary}
...
\textbf{Exact chromatic number and theta-perfectness.}  This is Theorems~\ref{thm:theta-gq} and~\ref{thm:chi-exact} and Corollary~\ref{cor:theta-perfect}.  The equivalence for Conjecture~\ref{conj:linear} is Remark~\ref{rem:theta-evidence}(3).
\end{proof}
...
\bibitem{Lovasz1979}
L.~Lov\'asz,
\textit{On the Shannon capacity of a graph},
IEEE Transactions on Information Theory \textbf{25} (1979), 1--7.
